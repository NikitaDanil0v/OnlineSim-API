...
def startWorker(api_key, num):
    ...
    print("Onlinesim: Start worker on number:" + num)
            ...
            if sim.LastJson != {'response': 'ERROR_NO_OPERATIONS'}:
                for account in list(sim.LastJson):
                    if str(account['response']) != 'TZ_INPOOL':
                        ...
                        if (account['number'] not in db_workflow['Number']) or (str(account['response']) == 'TZ_NUM_ANSWER' and int(account['time']) <= 900):
                        ...
                        if account['number'] in db_accounts['Number']: sim.setOperationOk(api_key, account['tzid'])
        sleep(1)
